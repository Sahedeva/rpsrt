...
Router.onBeforeAction(mustBeSignedIn, {except: ['about', 'history']});
...
Router.route('/human', {
    ...
    var check1 = Players.find({userId: currentUserId}).fetch();
    var opp_id = player['opponent_id'];
		var opp = Players.findOne({userId: opp_id});
		if (check1.length==1) {
			return { 
			player: player,
			opp: opp
...
Router.route('/lobby', {
	...
	data: function(){
		// var currentUserId = Meteor.userId();
		// var currentPlayer = Players.findOne({userId: currentUserId}, {sort: {submitted: -1, limit: 1}});
		// var id = currentPlayer._id;
		var player = {lobby: true, challenge: 'show', accept: 'hidden', reject: 'hidden', game: 'hidden'};
    console.log(player);
...
Router.route('/computer', {
	...
	data: function(){
		...
		// var test = Players.find({active: true}, {limit: 2}).fetch();
		// console.log("players object from computer route: "+test);
		return { players: players};
	}
});
   //      var currentUserId = Meteor.userId();
...
Router.route('/hal', {
	...
	data: function(){
     ...
     //    	rock_url: opponent_rock,
     //    	paper_url: opponent_paper,
     //    	scissors_url: opponent_scissors,
     //    	win: 0,
					// loss: 0,
					// tie: 0,
					// wlt: "",
